...
import { exists, listFiles, readTextOrNull, reconcileMirror, removeEmptyParents, writeText, } from "../lib/fsx.js";
...
export async function sync(options) {
    ...
    for (const file of files) {
        ...
        if ((await readTextOrNull(absolute)) !== file.contents)
    ...
    for (const mirror of mirrors) {
        const result = await reconcileMirror(mirror.from, path.join(options.root, mirror.to), manifest.mirrored[mirror.to] ?? []);
 ...
 */
function refuse(version, wrote, packFiles, generated) {
    console.error(`agentkit ${version} is older than the agentkit that last wrote this repo's ` +
        `agent config${wrote === undefined ? "" : ` (${wrote})`}, so nothing was written.`);
    for (const file of [...packFiles, ...generated])
        console.error(`  ${file}`);
 ...
 */
async function pruneGeneratedDirs(root, dirs, files) {
 ...
 */
async function pruneDisabledVendors(canonical, manifest) {
    const removed = [];
    const foreign = [];
...
async function emptyMirror(dest, previous) {
...
function report(canonical, emitFor, changed, removed, foreign, mirrorDirs) {
    ...
    if (foreign.length > 0) {
        ...
        for (const file of foreign)
            console.log(`            ${file}`);
        console.log("            These sit in a generated directory but are yours, so they\n" +
