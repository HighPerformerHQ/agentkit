...
async function scratch() {
    ...
    await fs.writeFile(path.join(root, ".agents", "agentkit.config.json"), JSON.stringify({ packs: ["core"], vendors: ["claude"] }, null, 2));
    return root;
}
const agentsPath = (root, rel) => path.join(root, ".agents", rel);
const packPath = (rel) => path.join(packageRoot(), "packs", "core", rel);
async function actionFor(root, rel, reseed = false) {
...
test("a pack change reaches a file the user never touched", async () => {
    ...
    await fs.writeFile(agentsPath(root, "agentkit-manifest.json"), JSON.stringify(manifest, null, 2));
...
test("a read-only load writes nothing at all", async () => {
    ...
    assert.equal(canonical.packPlan.find((entry) => entry.path === TRACKED)?.action, "seed", "it should still report the pending work");
