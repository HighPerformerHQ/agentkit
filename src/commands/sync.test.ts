...
async function scratch(vendors: Vendor[] = ["claude", "codex", "opencode"]): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentkit-sync-"));
  await fs.mkdir(path.join(root, ".agents"), { recursive: true });
  await fs.writeFile(
    path.join(root, ".agents", "agentkit.config.json"),
    JSON.stringify({ packs: ["core"], vendors }, null, 2),
  );
  await sync({ root, quiet: true });
  return root;
}

async function setVendors(root: string, vendors: Vendor[]): Promise<void> {
  await fs.writeFile(
    path.join(root, ".agents", "agentkit.config.json"),
    JSON.stringify({ packs: ["core"], vendors }, null, 2),
  );
