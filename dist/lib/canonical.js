...
import { copyDirIfAbsent, exists, glob, isDir, parseFrontmatter, readText, readTextOrNull, writeText, } from "./fsx.js";
 ...
 */
export async function loadCanonical(root) {
    ...
    for (const pack of config.packs) {
        ...
        for (const kind of ["skills", "commands", "rules"]) {
            await copyDirIfAbsent(path.join(packDir, kind), path.join(agentsDir, kind), root);
        }
    }
    ...
    return {
        ...
        mcp: JSON.parse(await readText(mcpPath)),
        foreignBlocks: extractForeignBlocks(await readTextOrNull(path.join(root, "AGENTS.md"))),
    };
}
...
export async function loadConfig(agentsDir) {
...
async function readRules(agentsDir) {
 ...
 * each other forever.
 */
export function extractForeignBlocks(existing) {
    if (existing === null)
        return [];
