...
import { ALL_VENDORS, type Vendor } from "./lib/types.js";
...
function parseVendors(raw: string | undefined): Vendor[] | undefined {
  if (!raw) return undefined;
  const requested = raw.split(",").map((v) => v.trim()).filter(Boolean);
  const invalid = requested.filter((v) => !ALL_VENDORS.includes(v as Vendor));
  if (invalid.length > 0) {
    throw new Error(
      `Unknown vendor(s): ${invalid.join(", ")}. Valid: ${ALL_VENDORS.join(", ")}`,
    );
  }
  return requested as Vendor[];
}
