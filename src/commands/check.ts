...
export async function check(root: string): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (actual === null) stale.push(`${file.path} (missing)`);
    else if (actual !== file.contents) stale.push(`${file.path} (out of date)`);
  }
...
function reportPendingPackWork(plan: { path: string; action: string }[]): void {
  ...
  if (pending.length > 0) {
    console.log(
      `agentkit check: ${pending.length} pack file(s) have updates available - run \`agentkit sync\``,
    );
    for (const entry of pending) console.log(`  .agents/${entry.path}`);
  }
  if (conflicts.length > 0) {
    console.log(
      `agentkit check: ${conflicts.length} pack file(s) changed upstream but are locally edited`,
    );
    for (const entry of conflicts) console.log(`  .agents/${entry.path}`);
  }
}
...
async function diffTree(
  ...
  for (const relative of sourceFiles) {
    ...
    if (!a.equals(b)) problems.push(`${label}/${relative} (out of date)`);
  }
